...
fn main() {
    ...
    let message = b"alv-around";

    /* Enter solution here */

    /*
     *   rogue-key attacks, which can be
     *   mounted whenever adversaries are allowed to choose their public keys arbitrarily. Typical attacks
     *   have the adversary use a public key that is a function of an honest user’s key, allowing him to
     *   produce forgeries easily.
     */
    // PART I -> find the acc pk and signature that makes bls_verify pass
    let agg_pks = public_keys
        .iter()
        .fold(G1Projective::zero(), |acc, (pk, _)| acc + pk)
        .into_affine();

    let new_key: G1Affine = agg_pks.neg();
    let aggregate_signature: G2Affine = G2Affine::zero();

    // Part II -> create a proof that pass pok_verify
    use ark_ff::Field;
    let agg_proof: G2Affine = public_keys
        .iter()
        .enumerate()
        .fold(G2Projective::zero(), |acc, (i, (_, pi))| {
            let inv = Fr::from(i as u64 + 1).inverse().unwrap();
            let factor = Fr::from(10) * inv;
            acc + pi.mul(factor)
        })
        .into_affine();
    let new_proof: G2Affine = -agg_proof;
...
#[test]
fn test_assumption() {
    let r_g2 = derive_point_for_pok(0);
    assert_eq!(r_g2.mul(Fr::from(8 + 1)), derive_point_for_pok(8));

    let public_keys: Vec<(G1Affine, G2Affine)> = from_file("public_keys.bin");
    let sum_index: Fr = (0..public_keys.len()).map(|x| Fr::from(x as u64 + 1)).sum();
    assert_eq!(sum_index, Fr::from(45u64));
}
